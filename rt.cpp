...
void rt::dosrt(std::vector<process> arr, int arrCount) {
    ...
    avg_turnaround_time = total_turnaround_time / passed;
    avg_waiting_time = total_waiting_time / passed;
...
void rt::dohrt(std::vector<process> arr, int arrCount) {
    bool ProcsCompleted = false;
    int completed = 0;
    long avg_turnaround_time;
    long avg_waiting_time;
            ...
            if (waiting.size() > 0) {
                while (1) {
                    ...
                    if (waiting[max].deadline < waiting[max].burst + currentTime) {
                        ...
                        completed++;
                        failed++;
						cout<<"Scheduling Error Detected\n";
						fail=true;
						break;
                    } else {
                        break;
